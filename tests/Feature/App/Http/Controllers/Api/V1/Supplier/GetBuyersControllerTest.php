...
class GetBuyersControllerTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function test_supplier_can_get_related_buyers_successfully()
    {
        ...
        $order1 = $this->createOrder($buyer1, $supplier, ['total' => 150.00, 'total_products' => 10]);
        $this->createOrderProduct($order1, 10, 15.00);

        $order2 = $this->createOrder($buyer1, $supplier, ['total' => 200.00, 'total_products' => 5]);
        $this->createOrderProduct($order2, 5, 40.00);

        // Create order for buyer2
        $order3 = $this->createOrder($buyer2, $supplier, ['total' => 100.00, 'total_products' => 3]);
        $this->createOrderProduct($order3, 3, 33.33);

        $token = $supplier->createToken('test')->plainTextToken;

        $response = $this->withToken($token)->getJson(route('suppliers.buyers.index'));
    ...
    public function test_supplier_can_sort_buyers_by_quantity()
    {
        ...
        $order1 = $this->createOrder($buyer1, $supplier, ['total_products' => 5]);
        $this->createOrderProduct($order1, 5); // Low quantity

        $order2 = $this->createOrder($buyer2, $supplier, ['total_products' => 15]);
    ...
    {
        $response = $this->getJson(route('suppliers.buyers.index'));
